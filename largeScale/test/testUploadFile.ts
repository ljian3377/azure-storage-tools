...
export async function main() {
  ...
  if (getRes.contentLength !== expectedLength) {
    throw new Error(`blob size don't match. uploaded: ${expectedLength}, downloaded: ${getRes.contentLength}`);
  }
  if (md5.compare(getRes.contentMD5) !== 0) {
    throw new Error(`md5 don't match. uploaded: ${md5}, downloaded: ${getRes.contentMD5}`);
  }
}

...
import { DataLakeServiceClient, StorageSharedKeyCredential, AnonymousCredential } from "@azure/storage-file-datalake";
...
async function main() {
  ...
  console.log(`Upload file ${fileName} successfully at ${new Date()}`, uploadRes.requestId);
  ...
  if (md5 && Buffer.from(md5).compare(getRes.contentMD5) !== 0) {
    throw new Error(
